                ...
                var ComboMenu = new Menu("连招", "Combo");
                {
                    var HealMenu = new Menu("治疗 (W)", "Heal");
                    {
                        foreach (var Obj in ObjectManager.Get<Obj_AI_Hero>().Where(i => i.IsAlly))
                        {
                            ...
                            AddItem(HealMenu, Name + "HpU", "-> 如果血量低于", 40);
                        }
                        ComboMenu.AddSubMenu(HealMenu);
                    }
                    var SaveMenu = new Menu("保留 (R)", "Save");
                    {
                        foreach (var Obj in ObjectManager.Get<Obj_AI_Hero>().Where(i => i.IsAlly))
                        {
                            var Name = Obj.IsMe ? "Self" : Obj.ChampionName;
                            AddItem(SaveMenu, Name, Name);
                            AddItem(SaveMenu, Name + "HpU", "-> 如果血量低于", 30);
                        }
                        ComboMenu.AddSubMenu(SaveMenu);
                    }
                    var AntiMenu = new Menu("反突进 (R)", "Anti");
                    {
                    ...
                    AddItem(ComboMenu, "EAoE", "->AOE消耗目标");
                    AddItem(ComboMenu, "R", "使用 R");
                    AddItem(ComboMenu, "RSave", "-> 保留");
                    AddItem(ComboMenu, "RAnti", "-> 遇到危险的大招", new[] { "关闭", "自己", "盟友", "两者都是" }, 3);
